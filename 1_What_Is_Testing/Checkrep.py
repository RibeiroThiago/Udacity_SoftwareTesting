...
#
# Modified by Thiago Figueiró at 21/11/2015 00:43
# - Modified the 'checkRep' method
...
class Queue:
    ...
    def checkRep(self):     
    # When the program starts, it MUST have a size bigger than 0 and
    # smaller than the max size of it.
        assert self.size >= 0 and self.size <= self.max
        
    # After queue, the tail will be different than head and the difference
    # MUST be the size of the array
        if self.tail > self.head:
            assert (self.tail - self.head == self.size)
        if self.tail < self.max:
            assert (self.max - self.tail == self.size)
            
    # However, if the tail is equal to the head, then the size either is zero
    # or the size is the maximum size of the array
        if self.tail == self.head:
